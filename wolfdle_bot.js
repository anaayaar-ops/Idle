...
// ==================================================
// محاكاة رد اللعبة (نفس منطق Wordle القياسي مع تكرار الحروف):
// بتاخد تخمين وكلمة "افتراضية" وتحسب إيه هيكون رد اللعبة عليها
// (correct / present / absent) لكل خانة. دي أساس منطق التمييز تحت.
// ==================================================
function simulateFeedback(guess, answer) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const idx = answerLetters.findIndex((L, j) => L === guessLetters[i] && !answerUsed[j]);
    if (idx !== -1) {
      result[i] = 'present';
      answerUsed[idx] = true;
    }
...
// ==================================================
// اختيار "تخمين تمييزي" (discriminating guess): لما يكون فيه أكتر من كلمة
// محتملة تطابق القيود الحالية، بدل ما نراهن بالعمى على واحدة ونخسر لو غلط،
// بندوّر بين نفس الاحتمالات على الكلمة اللي لو جرّبناها هتقسّم باقي
// الاحتمالات لأكبر عدد مجموعات ممكنة (يعني هتديّنا أكبر قدر من المعلومات
// الجديدة أيًا كان رد اللعبة عليها) وفي نفس الوقت تقلل "أسوأ سيناريو"
// (أكبر مجموعة ممكن تفضل بعدها). ده بالظبط معنى "نجرب حرف من كل احتمال".
// ==================================================
function chooseDiscriminatingGuess(candidates) {
  if (candidates.length <= 1) return null;

  let best = null;
  let bestWorstCase = Infinity;
  let bestGroupCount = -Infinity;

  for (const guess of candidates) {
    const groups = {};
    for (const answer of candidates) {
      const pattern = simulateFeedback(guess, answer);
      groups[pattern] = (groups[pattern] || 0) + 1;
    }
    const groupValues = Object.values(groups);
    const worstCase = Math.max(...groupValues);   // أسوأ حالة: أكبر مجموعة ممكن تفضل لو الرد جه بالشكل ده
    const groupCount = groupValues.length;        // عدد الاحتمالات المختلفة اللي التخمين ده بيميز بينها

    if (worstCase < bestWorstCase || (worstCase === bestWorstCase && groupCount > bestGroupCount)) {
...
// الاحتمالات بدل ما يجرب عشوائي ويخاطر بالخسارة.
function pickBestFromCandidates(candidates, testedLetters, attemptsLeft) {
  if (candidates.length === 1 || attemptsLeft <= 1) {
    return candidates[0];
  }

  const discriminator = chooseDiscriminatingGuess(candidates);
  if (discriminator) return discriminator;

  let best = candidates[0], bestScore = -Infinity;
  for (const w of candidates) {
    const s = wordScore(w, testedLetters);
...
function pickNextGuess(rows) {
  ...
  // فيه أكتر من احتمال ممكن يكون هو الكلمة الصحيحة، ولسه فيه وقت كفاية.
  // بدل ما نجرب واحد بس ونخاطر بالخسارة، ندوّر على تخمين يفرّق بين الاحتمالات.
  const discriminator = chooseDiscriminatingGuess(candidates);
  if (discriminator) {
    console.log(`🎯 فيه ${candidates.length} احتمال محتمل - هجرب كلمة تميّز بين الاحتمالات (بدل ما نراهن بالعمى): ${discriminator}`);
