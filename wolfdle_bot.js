...
let isSending = false;
let pendingRecheck = false;  // تحديث للوحة وصل أثناء الإرسال ولازم يتفحص تاني
...
function pickNextGuess(rows) {
  ...
  if (candidates.length === 0) {
    ...
    const partial = DICTIONARY
      ...
      .sort((a, b) => b.s - a.s)[0];
    if (!partial || partial.s === -Infinity) {
      console.log('❌ حتى القاموس الاحتياطي مفيهوش كلمة متوافقة مع القيود المؤكدة — القاموس ناقص الكلمة الصحيحة على الأرجح.');
      return null;
    }
    return partial.w;
  }
...
// نتيجة تقريبية لكلمة مقابل القيود المؤكدة: أي مخالفة لقيد مؤكد (حرف لازم يكون
// موجود، أو حرف لازم يكون مستبعد) تُقصي الكلمة تمامًا (-Infinity) بدل ما تتقيّم
// بدرجة أقل بس تفضل مرشّحة — عشان ميترشحش تخمين بيخالف معلومة مؤكدة زي "تهديد".
...
async function sendGuess(word) {
  ...
  try {
    ...
    setTimeout(() => {
      isSending = false;
      // لو وصل تحديث للوحة أثناء الإرسال واتجاهل، نراجعه دلوقتي بدل ما يضيع للأبد
      if (pendingRecheck) {
        pendingRecheck = false;
        recheckBoard();
      }
...
async function recheckBoard() {
  ...
  if (gameRows.length === 0 || gameRows.length >= 6) return;
...
async function handleWolfdleMessage(message) {
  ...
  // لو البوت لسه بيبعت تخمين سابق، منسيبش التحديث ده يضيع — نأجّل المراجعة
  // لحد ما الإرسال يخلص بدل التجاهل النهائي القديم
